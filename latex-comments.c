...
{
    /* Step 1: Set metadata */
    plugin->info->name = "Latex-Comments";
    plugin->info->description = "Displays latex-comments as graphic";
    plugin->info->version = "0.0";
    plugin->info->author = "Michelle Rausch <michelle.rausch@protonmail.com>";
 
    /* Step 2: Set functions */
    plugin->funcs->init = latexcomments_init;
    plugin->funcs->cleanup = latexcomments
    _cleanup;
